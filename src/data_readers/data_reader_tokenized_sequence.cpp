...
tokenized_sequence_data_reader::tokenized_sequence_data_reader(const tokenized_sequence_data_reader& rhs)  : data_reader_sample_list(rhs) {
  copy_members(rhs);
}

tokenized_sequence_data_reader::~tokenized_sequence_data_reader() {}

tokenized_sequence_data_reader& tokenized_sequence_data_reader::operator=(const tokenized_sequence_data_reader& rhs) {
...
void tokenized_sequence_data_reader::copy_members(const tokenized_sequence_data_reader &rhs) {
...
void tokenized_sequence_data_reader::load() {
...
void tokenized_sequence_data_reader::do_preload_data_store() {
  double tm1 = get_time();
  if (get_comm()->am_world_master()) {
  ...
  for (std::unordered_map<std::string, std::map<size_t, size_t>>::const_iterator fn_iter = m_local_to_index.begin(); fn_iter != m_local_to_index.end(); ++fn_iter) {
  ...
  // Create a buffer for reading in the TOKENIZED_SEQUENCE files
  ...
  for (const auto &filename : my_ordering) {
    ...
    for (size_t k=0; k<loadme.size(); k++) {
      ...
      if (iter == m_sample_offsets.end()) {
        LBANN_ERROR("failed to find ", index, " in m_sample_offsets map; map size: ", m_sample_offsets.size());
      }
      ...
      //dah - 'length' is length of the TOKENIZED_SEQUENCE string, not necessarily
      ...
      if (k+1 < loadme.size()) {
        ...
        offset_map_t::const_iterator next_iter = m_sample_offsets.find(next_index);
      ...
      // of the string
      size_t read_len = max_offset + len_of_last_offset - min_offset + sizeof(char);
      if((read_len >= (buf_size - (m_linearized_data_size + sizeof(char)))) || local == local_to_index.rbegin()->first) {
...
std::set<int> tokenized_sequence_data_reader::get_my_indices() const {
  std::set<int> s;
...
bool tokenized_sequence_data_reader::fetch_datum(Mat& X, int data_id, int mb_idx) {
  ...
  const conduit::unsigned_short_array data = node["/" + LBANN_DATA_ID_STR(data_id) + "/data"].as_unsigned_short_array();
  size_t j;
...
bool tokenized_sequence_data_reader::fetch_label(Mat& Y, int data_id, int mb_idx) {
  LBANN_ERROR("tokenized_sequence_data_reader::fetch_label is not implemented");
  return true;
}

bool tokenized_sequence_data_reader::fetch_response(Mat& Y, int data_id, int mb_idx) {
  LBANN_ERROR("tokenized_sequence_data_reader::fetch_response is not implemented");
  return true;
}


//user feedback
void tokenized_sequence_data_reader::print_statistics() const {
  if (!get_comm()->am_world_master()) {
    return;
  }
  ...
  //std::cerr << "mem for data, lower bound: " << utils::commify(get_mem_usage()) << std::endl;
...
void tokenized_sequence_data_reader::load_vocab(std::string fn) {
...
void tokenized_sequence_data_reader::load_offsets_and_lengths() {
...
std::string tokenized_sequence_data_reader::get_raw_sample(std::istream* istrm, size_t index, size_t buf_offset) {
  offset_map_t::const_iterator iter = m_sample_offsets.find(index);
  if (iter == m_sample_offsets.end()) {
    LBANN_ERROR("failed to find ", index, " in m_sample_offsets map; map size: ", m_sample_offsets.size());
  }
  ...
  if (start) {
    ...
    if (c != '\n') {
      ...
      s << "string does not start after a newline; previous char, as int: \n"
        << "  offset: "<<offset<<" Length: " << length << " as int: "
        << (int)c << " tellg: " << istrm->tellg();
      if (isprint(c)) {
        s << " as char: " << c;
      } else {
  ...
  std::string tokenized_sequence_str;
  tokenized_sequence_str.resize(length);
  istrm->read((char*)tokenized_sequence_str.data(), length);
  ...
  if (! (istrm->eof() || is_delimiter(c2)) ) {
    ...
    s << "string does not appear to be followed by a whitespace (or valid delimiter); "
      << "the string: >>>"  << tokenized_sequence_str << "<<<; next char, as int: "
      << c2 << "; tellg: " << istrm->tellg();
    if (isprint(c2)) {
      s << " as char: " << (char)c2;
    } else {
      s << " not a printable character";
  ...
  // if found
  size_t TOKENIZED_SEQUENCE_len = tokenized_sequence_str.length();
  for(std::string::iterator it = tokenized_sequence_str.begin(); it != tokenized_sequence_str.end(); ++it) {
    auto &c = *it;
    if (is_delimiter(c)) {
      TOKENIZED_SEQUENCE_len = std::distance(tokenized_sequence_str.begin(), it);
      break;
    }
  }
  return tokenized_sequence_str.substr(0, TOKENIZED_SEQUENCE_len);
}

void tokenized_sequence_data_reader::build_some_maps() {
...
void tokenized_sequence_data_reader::read_offset_data(std::vector<SampleData> &data) {
  data.clear();
  ...
  for (size_t j=0; j<data_filenames.size(); j++) {
    if (m_filename_to_local_id_set.find(data_filenames[j]) != m_filename_to_local_id_set.end()) {
      ...
      for (const auto &local_id : indices) {
        ...
        if((read_len >= (buf_size - OffsetAndLengthBinarySize)) || local_id == *(indices.rbegin())) {
          ...
          for (const auto& r_local_id : indices_in_range) {
            if(!buf_stream.seekg(r_local_id*OffsetAndLengthBinarySize-start_offset)) {
...
void tokenized_sequence_data_reader::read_metadata_file(
  std::vector<size_t>& samples_per_file,
  std::vector<std::string>& data_filenames,
  std::vector<std::string>& offsets_filenames) {
  ...
  for (size_t j=0; j<data_filenames.size(); j++) {
    ...
    // Quick test: does the number of state samples match the count from the offsets file?
    // This is mostly to detect off-by-one, or other, errors in the sample list
    if (num_samples_cur_file != n_samples) {
      LBANN_ERROR("total num samples in ", data_filenames[j], " from offsets file is ", num_samples_cur_file, " but metadata file claimed the number is ", n_samples);
    }
    // if (m_filename_to_local_id_set.find(data_filename) == m_filename_to_local_id_set.end()) {
    //   LBANN_WARNING("Unused metadata mapping: failed to find ", data_filename, " in m_filename_to_local_id_set; ", " num keys: ", m_filename_to_local_id_set.size());
    // }
  }
  // END ======= sanity checks =======
}

void tokenized_sequence_data_reader::use_unused_index_set(execution_mode m) {
...
void tokenized_sequence_data_reader::get_sample_origin(
  ...
  std::unordered_map<size_t, std::string>::const_iterator t3 = m_index_to_filename.find(index_in);
