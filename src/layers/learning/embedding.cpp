...
void fp_compute_impl(embedding_layer<TensorDataType, data_layout::DATA_PARALLEL,El::Device::CPU>& l) {
  ...
  // Local data
  const auto& local_dict = dynamic_cast<const El::Matrix<TensorDataType, El::Device::CPU>&>(l.get_data_type_weights()[0]->get_values().LockedMatrix());
...
void bp_compute_impl(embedding_layer<TensorDataType, data_layout::DATA_PARALLEL,El::Device::CPU>& l) {
  ...
  opt.add_to_gradient(l.m_dictionary_gradient, TensorDataType{1}, true);
